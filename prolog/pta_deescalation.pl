...
% The collective driver loads pta_ontology.pl before this file.
...
% Generic literal equivalence: requires complete truth vectors over
% example_domain. The session does not carry literal transform metadata, so it
% must not strengthen this observation into "thresholds equivalent".
literals_equivalent(L1, L2) :-
    L1 \= L2,
...
clause_subsumes(C1, C2) :-
    ...
    % Strict containment: equal literal sets are equivalent/redundant, not
    % mutual subsumption.
    \+ \+ (clause_literal(C2, Extra), \+ clause_literal(C1, Extra)),
