...
## Milestone 7: PTA reasoning/control plane — proposed

> Design: `docs/pta-control-plane.md` — two-plane architecture where the three PTA classes (Input, De-escalation, Escalation) are the **control and reasoning layer around the native Tsetlin substrate**. Native clause banks/TA states/weights/bit-planes/SIMD/CUDA remain the fast executable plane; every deployable result crosses an exact lowering gate as a `PTAEscalationProposal`. Trust boundary preserved (bounded search → independent oracle → shadow audit → publish, with reopen), but Prolog is now free to use unification, CLP(FD), sets, DCGs, and richer hypothesis language during deliberation.
>
> Implementation status: scaffolds/reference prototypes exist; full Prolog-driven collective + exact lowering + native execution are roadmap work (see checklist).

- [x] Typed `PTAEscalationProposal` + shared ontology (`observation/4`, `insight/4`, `proposal/3`, `lowerable/2`) — `python/prolog_tsetlin/pta/` + `prolog/pta_*.pl` compiled modules; Python `PTAReasoningSession` collective boundary.
- [x] Exact `lowerable()` → `lower_exact()` + `syntactically_bounded()` — bounded literal/clause/graph-depth/patch-cells/weight ranges, unbounded recursion → `not lowerable`; only `lower_exact()` success means native candidate. Canonical example `temperature 71–76 ∧ mode=manual ∧ previous=B → 104∧105∧231∧388`.
- [x] Input-PTA learned Booleanization with provenance/retirement — reference oracle with pure preview (no catalog mutation), inclusive flags preserved (`pta/input.py:InputPTA` via `LiteralCatalog.preview_*`).
- [ ] De-escalation Type-III-inspired + reversible absorption — reference oracle for literal/clause redundancy; exact Type III feedback via `prolog/pta_deescalation.pl` is roadmap.
- [ ] Shared weighted clause bank / CoTM — reference oracle `pta/escalation.py:allocate_cotm_weights` (greedy) + exact `lower_exact()` check; native CoTM matrix + Prolog CLP(FD) solver are roadmap.
- [ ] Sparse/indexed lowering — `pta/sparse.py:to_sparse_exact()` exact (dense→sparse same literals), `propose_sparse_morphology()` → PTA proposal → oracle/shadow lineage (previous `lower_to_sparse` conflated exact vs morphology).
- [ ] Focused Negative Sampling and multigranularity — reference oracles `pta/sampling.py` strict int/bool/range checks; native masks/policies wiring is roadmap.
- [ ] Regression, CTM, Graph TM, sequence and composite TM — scaffold/reference oracles with exact lowering stubs (`pta/regression.py`, `convolutional.py`, `graph_pta.py`, `sequence.py`, `composite.py` — no synthetic `hash()` IDs, validated inputs, no clamping across exact boundary, `smallest_specialist_subset` exact for ≤20).
