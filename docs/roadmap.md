...
- [x] Sparse/indexed lowering from de-escalation knowledge — `pta/sparse.py:lower_to_sparse` removes thresholds_equivalent, unused, duplicate/subsumed clauses → `SparseClauseBank`/`ClauseIndex` with `to_proposal_structure()` for native `SparseClause` dispatch.
- [x] Focused Negative Sampling and multigranularity as PTA-authored masks/policies — `pta/sampling.py` `fns_mask_from_confusable` → `negative_candidates[ class]=bitmask`, `fns_mask_from_counterexamples`, `multigranularity_schedule` → `clause→s` (`8.0` over-specific, `2.1` over-broad).
- [x] Regression, CTM, Graph TM, sequence and composite TM with PTA-assisted structure invention — `pta/regression.py` residual `71–76` → `regression_clause`, `pta/convolutional.py` `A above B/within-2` → `patch_clause`, `pta/graph_pta.py` `ancestor` depth 3 → `graph_clause` (unbounded recursion not lowerable), `pta/sequence.py` `A followed by B` → `logic_program`, `pta/composite.py` `has_relation_structure→graph_model` gate + smallest subset cover.
