...
Verify the installed SDK as a clean CMake consumer:

```bash
cmake -S tests/consumer -B out/consumer \
  -DCMAKE_PREFIX_PATH=/absolute/path/to/PrologTsetlinMachine/out/install
cmake --build out/consumer --config Release
```

On Windows, run `out\consumer\ptm_consumer_smoke.exe`; on a single-config
Linux/macOS build, run `out/consumer/ptm_consumer_smoke`.

The official CMake documentation explains the
[`cmake --install` workflow](https://cmake.org/cmake/help/latest/guide/tutorial/Installation%20Commands%20and%20Concepts.html).
...
See [CUDA packed-TM execution](docs/cuda-packed-tm.md) for the experimental
